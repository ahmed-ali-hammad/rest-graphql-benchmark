...
from patient_info_restapi.serializers import (
    AddressSerializer,
    DoctorSerializer,
...
class EmergencyContactViewSet(ModelViewSet):
    ...
    def get_patient_emergency_contact(self, request):
        ...
        emergency_contact = get_object_or_404(
            Patient, id=request.data.get("patient_id")
        ).emergency_contact

        return Response(
            EmergencyContactSerializer(emergency_contact).data,
...
class MedicalRecordViewSet(ModelViewSet):
    ...
    def get_patient_medical_records(self, request):
        ...
        medical_records = get_object_or_404(
            Patient, id=request.data.get("patient_id")
        ).medical_records.all()

        return Response(
            MedicalRecordSerializer(medical_records, many=True).data,
            status=status.HTTP_200_OK,
        )
...
class MedicationViewSet(ModelViewSet):
    ...
    def get_patient_medication(self, request):
        ...
        medical_records = (
            get_object_or_404(Patient, id=request.data.get("patient_id"))
            .medical_records.select_related("illnesses", "allergies")
            .filter(is_cured=False)
        )

        medications = []

        for medical_record in medical_records:
            if medical_record.illnesses:
                medications.append(medical_record.illnesses.medication)
            if medical_record.allergies:
                medications.append(medical_record.allergies.medication)

        return Response(
            MedicationSerializer(medications, many=True).data, status=status.HTTP_200_OK
        )
